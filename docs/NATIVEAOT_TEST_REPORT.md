...
合计 **1494 个框架内用例全部通过**。执行方式：主全量运行 1492 通过、2 个重启用例跳过；随后两个重启用例在独立运行中各通过 1 个、0 跳过，避免干扰并行的原生验收。TRX 校验器确认 9 个全量运行和 2 个独立故障运行均非空、无失败/中止。

证据：`artifacts/aot-210/release-jit`、`artifacts/aot-210/release-jit-recovery`。
